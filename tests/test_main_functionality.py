...
@allure.suite("Тесты основной функциональности главной страницы")
...
class TestMainFunctionality:
    ...
    def test_counter_increment_when_add_to_burger(self, driver):
        page = MainPage(driver)
        ...
        assert final_count > initial_count, \
            f"Счётчик ингредиента не увеличился: было {initial_count}, стало {final_count}"

    @allure.title("Оформление заказа")
    def test_place_order(self, driver, login_user):
        page = MainPage(driver)
        page.drag_ingredient_to_constructor(0)
        page.place_order()
